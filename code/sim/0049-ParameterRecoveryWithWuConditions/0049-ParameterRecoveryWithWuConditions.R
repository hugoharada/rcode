...
for(i in (1:test_n)){
  ...
  U2 =simdata(a=a,d=d,N=N,itemtype = '2PL', Theta = matrix(Theta[,2],ncol=1,nrow = length(Theta[,2])))
...
colnames(U1)=paste0("Item.",t1_index_range,".t1")
U2 =simdata(a=a,d=d,N=N,itemtype = '2PL', Theta = matrix(Theta[,2],ncol=1,nrow = length(Theta[,2])))
group <- c(rep("g1",nrow(U1)),rep("g2",nrow(U2)))
data.mg <- data.frame(rbind(U1,U2),group)
head(data.mg)

lavaan.model.def <-function(){}



lavaan.model.wup11 <-'

lv1 =~ lambda1.t1*Item.1.t1 + lambda2.t1*Item.2.t1 + lambda3.t1*Item.3.t1 + lambda4.t1*Item.4.t1
lv2 =~ lambda1.t2*Item.1.t2 + lambda2.t2*Item.2.t2 + lambda3.t2*Item.3.t2 + lambda4.t2*Item.4.t2
lv3 =~ lambda1.t3*Item.1.t3 + lambda2.t3*Item.2.t3 + lambda3.t3*Item.3.t3 + lambda4.t3*Item.4.t3
...
# lv1 ~~ lv.var.t1*lv1 +           lv2 +           lv3
# lv2 ~~                 lv.var.t2*lv2 +           lv3
# lv3 ~~                                 lv.var.t3*lv3

#orthogonal variances
lv1 ~~ lv.var.t1*lv1
lv2 ~~ lv.var.t2*lv2
lv3 ~~ lv.var.t3*lv3
...
mean1.t1 + lv.mean.t1 * lambda1.t1 == 1
mean2.t1 + lv.mean.t1 * lambda2.t1 == 1
mean3.t1 + lv.mean.t1 * lambda3.t1 == 1
mean4.t1 + lv.mean.t1 * lambda4.t1 == 1

lv.var.t1 == 1
lv.mean.t1==0

# 
# mean1.t2 + lv.mean.t2 * lambda1.t2 == 0
# mean2.t2 + lv.mean.t2 * lambda2.t2 == 0
# mean3.t2 + lv.mean.t2 * lambda3.t2 == 0
# mean4.t2 + lv.mean.t2 * lambda4.t2 == 0
# 
# mean1.t3 + lv.mean.t3 * lambda1.t3 == 0
# mean2.t3 + lv.mean.t3 * lambda2.t3 == 0
# mean3.t3 + lv.mean.t3 * lambda3.t3 == 0
# mean4.t3 + lv.mean.t3 * lambda4.t3 == 0
...
lavaan.model.est <-function(){}
  ...
  lavaan.model <- lavaan.model.wup11
  lavaan.model <- lavaan.model.wup13
  # no auto vars
