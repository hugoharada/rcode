...
for(i in seq(1,90,1)){# i items
  for(j in c(1,2,3)){ # j moments
    item.par.sim[i,j] <- lambda2[i,j]*1.7
  }
  item.par.sim[i,4] <- tau[i]*1.7
}
...
for(i in seq(1,90,1)){# i items
  for(j in c(1,2,3)){ # j moments
    item.par.sim[i,j] <- lambda2[i,j]*sqrt(theta_var[j])/sqrt(1-(t(lambda2[i,])%*%lambda2[i,])*theta_var[j])*1.7
    item.par.sim[i,4] <- (tau[i]-lambda2[i,j]*mu[j])/sqrt(1-t(lambda2[i,])%*%lambda2[i,]*theta_var[j])*1.7 #d
  }
}
